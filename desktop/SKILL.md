...
### Where this runs
On **Claude Code**, this skill writes directly into your project on disk: it surveys the repo, copies templates, `chmod +x`'s the hooks, and wires `.gitignore`. On **Claude Desktop / claude.ai**, Claude runs in a sandbox without access to your local repo — so it reads the bundled `templates/`, fills them in for your project, and hands you each file's contents (or a downloadable bundle) to drop into your repository and commit. The structure and rules below are identical on both surfaces.

When invoked, ask the user which mode to run:

1. **Bootstrap a new project** → install the full structure from the bundled `templates/` directory.
...
Just tell Claude which mode in plain language, e.g. *"bootstrap agents-discipline here"*, *"scaffold a spec for billing-webhooks"*, or *"set up a Codex review of this branch"*.

> **Slash commands:** The Claude Code *plugin* edition of this skill also ships matching slash commands — `/agents-init`, `/agents-spec <feature>`, and `/agents-review`. Those are Claude Code-only. On Claude Desktop, invoke the modes conversationally as above.
...
All file templates are bundled alongside this `SKILL.md` in the `templates/`
directory. Reference them by **relative path** (for example
`templates/specs/SPEC-TEMPLATE.md`) — Claude reads them from this skill's own
...
Copy from `templates/`. Adapt to the target project. Don't add structure the project doesn't earn.
