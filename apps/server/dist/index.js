...
import { utteranceSchema, validateScript } from "@voice-radio/schema";
...
import { getProfileControls, resolveEnvironmentalAudioControls } from "@voice-radio/audio-core";
import { assertBuiltInVoiceId, buildTtsInstructions, clampNumber, normalizeTtsInput, organicVariantHint, resolveVoiceProfile, roleFromSpeaker } from "@voice-radio/voice-core";
import { checkFfmpegAvailable, ffmpegMissingMessage, finalArtifactPaths, generateFinalSpectrograms, generateNasaReferenceSpectrogram, generateUtteranceSpectrogramPair, nasaReferencePaths, publicArtifactUrl, sanitizePathSegment, spectrogramPublicUrls, utteranceArtifactPaths } from "./audio/spectrogram.js";
...
export function parseRequestLimits() {
    ...
    if (hardStopUsd < 0)
        throw new Error("COST_HARD_STOP_USD must be >= 0");
...
function enforceCostGuard() {
    if (requestCount >= requestLimits.maxRequests)
        throw new Error("Request hard stop reached");
    if (estimatedSpend >= requestLimits.hardStopUsd)
        throw new Error("Estimated spend hard stop reached");
}
function reserveCostSlot() {
    enforceCostGuard();
    requestCount += 1;
}
function estimateSpend(amount) {
...
export async function resolveGeneratedPath(publicPath) {
...
async function copyIfSessionArtifact(srcPath, destPath) {
    if (!destPath)
        return;
    await fs.mkdir(path.dirname(destPath), { recursive: true });
    await fs.copyFile(srcPath, destPath);
}
async function preserveRawUtteranceArtifact(srcPath, sessionId, utteranceId) {
    if (!sessionId)
        return;
...
async function preserveProcessedUtteranceArtifact(srcPath, sessionId, utteranceId) {
...
app.post("/api/tts/utterance", async (req, res) => {
    try {
        if (!openaiKey)
            return res.status(400).json({ error: "OPENAI_API_KEY missing" });
        const { utterance, voice, voiceProfile, instructions, model, speed, responseFormat, utteranceIndex = 0, sessionId } = req.body;
        ...
        const profile = resolveVoiceProfile({ ...(voiceProfile || {}), ...(parsed.data.voice || {}), ...(explicitVoiceId ? { voiceId: explicitVoiceId } : {}) }, roleFromSpeaker(parsed.data.speaker));
        const voiceSelection = typeof voice === "object" && voice ? voice : profile.voiceId;
        ...
        if (format !== "wav")
            return res.status(400).json({ error: "voice-radio-poc currently supports wav output for audio processing" });
...
app.post("/api/tts/batch", async (req, res) => {
    ...
    for (let utteranceIndex = 0; utteranceIndex < utterances.length; utteranceIndex += 1) {
        try {
            ...
            if (failed[failed.length - 1].includes("hard stop"))
                break;
        }
    }
...
app.post("/api/tts/audition", async (req, res) => {
    try {
        if (!openaiKey)
            return res.status(400).json({ error: "OPENAI_API_KEY missing" });
        const { text = "Odyssey, this is CAPCOM. Confirm signal lock and proceed with vector correction.", voiceProfile = {}, voice, model, speed } = req.body;
        const explicitVoiceId = typeof voice === "string" ? assertBuiltInVoiceId(voice) : undefined;
...
app.post("/api/audio/process", async (req, res) => {
    try {
        const { inputPath, controls = {}, macro, environment, sessionId, utteranceId } = req.body;
...
app.post("/api/audio/stitch", async (req, res) => {
    try {
        ...
        const resolved = await Promise.all(processedPaths.map((p) => resolveGeneratedPath(p)));
        ...
        if (artifactPaths) {
            ...
            if (rawPaths.length) {
                await stitchWavs(await Promise.all(rawPaths.map((p) => resolveGeneratedPath(p))), artifactPaths.rawAudioPath, stitchGapMs);
            }
        }
        res.json({
...
app.post("/api/spectrogram/utterance", async (req, res) => {
    try {
        const { sessionId, utteranceId } = req.body;
        if (!sessionId || !utteranceId)
            return res.status(400).json({ error: "sessionId and utteranceId are required" });
    ...
    try {
        ...
        for (const file of files) {
            ...
            try {
                ...
                if (error instanceof Error && error.message === ffmpegMissingMessage)
                    throw error;
...
async function resolveNasaReferenceSource(nasaSlug, source) {
...
app.post("/api/spectrogram/nasa-reference", async (req, res) => {
    try {
        const { nasaSlug, source } = req.body;
        if (!nasaSlug)
            return res.status(400).json({ error: "nasaSlug is required" });
